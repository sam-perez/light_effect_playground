...
const updateLightEffect = ({ colorScheme, startTime, currentTime, paceInMS }) => {
  ...
  const rng = seedrandom(`beatsElapsed:${beatsElapsed}`);
  const firstIdx = Math.floor(rng() * colorsToPickFrom.length);
  const secondIdx = Math.floor(rng() * colorsToPickFrom.length);
